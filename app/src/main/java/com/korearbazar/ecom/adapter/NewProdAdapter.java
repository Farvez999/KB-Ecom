...
public class NewProdAdapter extends RecyclerView.Adapter<NewProdAdapter.MyViewHolder> {
    ...
    private ArrayList<ProdModel> newModelArrayList;
    Context context;
    ProdInterface prodInterface;
    static String a_slag=null;

    public NewProdAdapter(Context ctx, ArrayList<ProdModel> dataModelArrayList, ProdInterface prodInterface){
        this.context=ctx;
        inflater = LayoutInflater.from(ctx);
        this.newModelArrayList = dataModelArrayList;
    ...
    public void onBindViewHolder(final NewProdAdapter.MyViewHolder holder, final int position) {
        ...
        Picasso.get().load(JSONURL+ProdImgUrl+newModelArrayList.get(position).getThumbnail()).into(holder.iv);
        holder.name.setText(newModelArrayList.get(position).getName());


        holder.price.setText(newModelArrayList.get(position).getShowPrice());
        final String totalPrice = newModelArrayList.get(position).getShowPrice();

        holder.previous_price.setText(newModelArrayList.get(position).getShowPreviousPrice());
        final String totalPreviousPrice = newModelArrayList.get(position).getShowPreviousPrice();
...
//            ss.setSpan(strikethroughSpan,0,5, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    ...
    class MyViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener{
        ...
        public void onClick(View view) {
            ...
            ProdModel pmss=newModelArrayList.get(postion);
